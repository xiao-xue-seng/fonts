...
FONTS_TO_PROCESS = [
    {
        ...
        "input": PROJECT_ROOT / "temp" / "ttf-raw" / "NotoSerifCJKsc-VF.otf",
        "output": PROJECT_ROOT / "temp" / "ttf-to-next" / "Noto-Serif-SC-Punct.ttf",
        "fallback_input": SCRIPT_DIR / "NotoSerifCJKsc-VF.otf",
        "suffix_en": "Punct",
        "suffix_zh": "標點",
    },
    {
        "name": "Noto Serif TC 標點",
        "input": PROJECT_ROOT / "temp" / "ttf-raw" / "NotoSerifCJKtc-VF.otf",
        "output": PROJECT_ROOT / "temp" / "ttf-to-next" / "Noto-Serif-TC-Punct.ttf",
        "fallback_input": SCRIPT_DIR / "NotoSerifCJKtc-VF.otf",
        "suffix_en": "Punct",
        "suffix_zh": "標點",
    },
    {
        "name": "全字齊楷 標點",
        "input": PROJECT_ROOT / "temp" / "ttf-to-next" / "TW-Kai-Aligned.ttf",
        "output": PROJECT_ROOT / "temp" / "ttf-to-next" / "TW-Kai-Aligned-Punct.ttf",
        "fallback_input": SCRIPT_DIR / "TW-Kai-Aligned.ttf",
        "suffix_en": "Punct",
        "suffix_zh": "標點",
    },
]
...
def process_font(font_cfg, punct_unicode_range_str):
    ...
    input_path = Path(font_cfg["input"])
    output_path = Path(font_cfg["output"])
    print(f"\n==================================================")
    print(f"開始處理：{font_name}")

    # 檢查輸入檔案路徑 (若主要路徑不存在則嘗試 fallback 路徑)
    if not input_path.exists():
        fallback = Path(font_cfg.get("fallback_input", ""))
